import { Matrix4 as Te, Vector3 as Se, Quaternion as Qt, Scene as Ct, WebGLRenderer as Ti, sRGBEncoding as Si, PerspectiveCamera as vi, Group as zt } from "three-mindar";
...
import { CSS3DRenderer as Vd } from "three-mindar/addons/renderers/CSS3DRenderer.js";
...
class Vb extends tn {
  constructor(e) {
    if (super(), this.upstream = e, P().get("IS_BROWSER"))
      ...
      throw new Error("Node text decoding is not included in the GastroAR browser runtime");
  }
  summary() {
...
class hi extends pi {
  ...
  async iterator() {
    if (di(this.input) && P().get("IS_NODE"))
      throw new Error("Node file URLs are not included in the GastroAR browser runtime");
...
class Zb {
  constructor({
    ...
    this.container = e, this.imageTargetSrc = t, this.maxTrack = a, this.filterMinCF = o, this.filterBeta = l, this.warmupTolerance = p, this.missTolerance = m, this.ui = new Rd({ uiLoading: r, uiScanning: n, uiError: u }), this.userDeviceId = c, this.environmentDeviceId = d, this.shouldFaceUser = !1, this.scene = new Ct(), this.cssScene = new Ct(), this.renderer = new Ti({ antialias: !0, alpha: !0 }), this.cssRenderer = new Vd({ antialias: !0 }), this.renderer.outputEncoding = Si, this.renderer.setPixelRatio(window.devicePixelRatio), this.camera = new vi(), this.anchors = [], this.renderer.domElement.style.position = "absolute", this.cssRenderer.domElement.style.position = "absolute", this.container.appendChild(this.renderer.domElement), this.container.appendChild(this.cssRenderer.domElement), this._boundResize = this.resize.bind(this), window.addEventListener("resize", this._boundResize);
  }
  async start() {
    this.ui.showLoading(), await this._startVideo(), await this._startAR();
  }
  stop() {
    this.controller && this.controller.stopProcessVideo(), this.video && this.video.srcObject && this.video.srcObject.getTracks().forEach(function(t) {
      t.stop();
    }), this.video && this.video.remove(), window.removeEventListener("resize", this._boundResize);
  }
  switchCamera() {
